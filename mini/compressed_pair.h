...
namespace mini {
	...
	public:
		...
		template<typename std::enable_if<std::is_default_constructible<T1>::value && std::is_default_constructible<T2>::value, int>::type = 0>
		compressed_pair() {}

		template<typename OtherT1, typename OtherT2,
			typename std::enable_if<std::is_constructible<T1, OtherT1>::value && std::is_constructible<T2, OtherT2>::value, int>::type = 0>
			compressed_pair(OtherT1 first, OtherT2 second) : m_first(std::forward<OtherT1>(first)), m_second(std::forward<OtherT2>(second)) {}

		template<typename OtherT1, typename OtherT2,
			typename std::enable_if<std::is_constructible<T1, OtherT1>::value && std::is_constructible<T2, OtherT2>::value, int>::type = 0>
			compressed_pair(const compressed_pair<OtherT1, OtherT2>& other) : m_first(std::forward<OtherT1>(other.first)), m_second(std::forward<OtherT2>(other.second)) {}

		compressed_pair(compressed_pair&& other) noexcept : m_first(std::move(other.m_first)), m_second(std::move(other.m_second)) {}

		template<typename OtherT1, typename OtherT2,
			typename std::enable_if<std::is_assignable<T1, OtherT1>::value && std::is_assignable<T2, OtherT2>::value, int>::type = 0>
	...
	public:
		...
		template<typename OtherT1, typename OtherT2,
			typename std::enable_if<std::is_constructible<T1, OtherT1>::value && std::is_constructible<T2, OtherT2>::value, int>::type = 0>
			compressed_pair(OtherT1 first, OtherT2 second) : first_type(std::forward<OtherT1>(first)), m_second(std::forward<OtherT2>(second)) {}

		template<typename OtherT1, typename OtherT2,
			typename std::enable_if<std::is_constructible<T1, OtherT1>::value && std::is_constructible<T2, OtherT2>::value, int>::type = 0>
			compressed_pair(const compressed_pair<OtherT1, OtherT2>& other) : first_type(std::forward<OtherT1>(other.first)), m_second(std::forward<OtherT2>(other.second)) {}

		compressed_pair(compressed_pair&& other) noexcept : first_type(std::move(other.first())), m_second(std::move(other.m_second)) {}
