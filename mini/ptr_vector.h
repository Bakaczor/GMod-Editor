...
namespace mini {
	...
	*/
	...
	private:
		...
		using deleter_const_reference = typename std::conditional<
				std::is_rvalue_reference<deleter_type>::value, typename std::add_rvalue_reference<
					typename std::add_const<typename std::remove_reference<deleter_type>::type>::type
				>::type,
				typename std::conditional<
					std::is_reference<deleter_type>::value, typename std::add_lvalue_reference<
						typename std::add_const<typename std::remove_reference<deleter_type>>::type
					>::type, const deleter_type&
				>::type
			>::type;
	...
	public:
		...
		public:
			...
			ptr_ref(ptr_ref&& other) noexcept : m_myPair(std::move(other.m_myPair)) {}
		...
		private:
			...
			ptr_ref(ptr_vector& container, vector_iterator element) : m_myPair(container.get_deleter(), element) {}

			ptr_ref(deleter_reference dt, vector_iterator element) : m_myPair(dt, element) {}
		...
		private:
			...
			ptr_iterator(ptr_vector& container, vector_iterator element) : m_myPair(container.get_deleter(), element) {}

			ptr_iterator(deleter_reference dt, vector_iterator element) : m_myPair(dt, element) {}

			compressed_pair<deleter_type, vector_iterator> m_myPair;
		};

		ptr_vector() : m_myPair(deleter_type{}, vector_type(0)) {}

		//SFINAE seems borked

		explicit ptr_vector(const allocator_type& alloc, typename std::enable_if<std::is_default_constructible<deleter_type>::value, int>::type = 0) : 
			m_myPair(deleter_type{}, vector_type{ alloc }) {}

		explicit ptr_vector(deleter_reference dt) : m_myPair(dt, vector_type{}) {}

		ptr_vector(const allocator_type& alloc, deleter_reference dt) : m_myPair(dt, vector_type{ alloc }) {}

		ptr_vector(size_type count, const allocator_type& alloc = allocator_type{}, typename std::enable_if<std::is_default_constructible<deleter_type>::value, int>::type = 0) :
			m_myPair(deleter_type{}, vector_type{ count, nullptr, alloc }) {}

		ptr_vector(size_type count, deleter_reference dt) : m_myPair(dt, vector_type{ count, nullptr }) {}

		ptr_vector(size_type count, const allocator_type& alloc, deleter_reference dt) : m_myPair(dt, vector_type{ count, nullptr, alloc }) {}
		...
		ptr_vector(InputIt first, InputIt last, const allocator_type& alloc = allocator_type{}) : m_myPair(deleter_type{}, vector_type{ first, last, alloc }) {}
		...
		ptr_vector(InputIt first, InputIt last, deleter_reference dt) : m_myPair(dt, vector_type{ first, last }) {}

		template<typename InputIt, typename std::enable_if<std::is_constructible<value_type, decltype(*std::declval<InputIt>())>::value, int>::type = 0>
		ptr_vector(InputIt first, InputIt last, const allocator_type& alloc, deleter_reference dt) : m_myPair(dt, vector_type{ first, last, alloc }) {}

		template<typename InputIt, typename std::enable_if<
			std::is_default_constructible<deleter_type>::value && std::is_same<unique_ptr_type&&, decltype(*std::declval<InputIt>())>::value, int>::type = 0>
		ptr_vector(InputIt first, InputIt last, const allocator_type& alloc = allocator_type{}) : m_myPair(deleter_type{}, vector_type( std::distance(first, last), nullptr, alloc )) {
		...
		ptr_vector(InputIt first, InputIt last, deleter_reference dt) : m_myPair(dt, vector_type{ std::distance(first, last), nullptr}) {
			_takeOver(_myVector().begin(), first, last);
		}

		template<typename InputIt, typename std::enable_if<std::is_same<unique_ptr_type&&, decltype(*std::declval<InputIt>())>::value, int>::type = 0>
		ptr_vector(InputIt first, InputIt last, const allocator_type& alloc, deleter_reference dt) : m_myPair(dt, vector_type{ std::distance(first, last), nullptr, alloc }) {
			_takeOver(_myVector().begin(), first, last);
		}
		...
		ptr_vector(std::initializer_list<value_type> init, const allocator_type& alloc = allocator_type{},
			typename std::enable_if<std::is_default_constructible<deleter_type>::value, int>::type = 0) : m_myPair(deleter_type{}, vector_type{ init, alloc }) {}

		//assumes ownership of all pointers
		ptr_vector(std::initializer_list<value_type> init, deleter_reference dt) : m_myPair(dt, vector_type{ init }) {}

		//assumes ownership of all pointers
		ptr_vector(std::initializer_list<value_type> init, const allocator_type& alloc, deleter_reference dt) : m_myPair(dt, vector_type{ init, alloc }) {}

		ptr_vector(const ptr_vector& other) = delete;

		ptr_vector(ptr_vector&& other) = default;

		ptr_vector(ptr_vector&& other, const allocator_type& alloc): m_myPair(std::move(other.get_deleter()), vector_type(std::move(other._myVector()), alloc)) {}
	...
	private:
		...
		template <typename InputIt, typename std::enable_if<
			std::is_default_constructible<_Dx>::value && std::is_same<std::unique_ptr<T, _Dx>&&, decltype(*std::declval<InputIt>())>::value, int>::type = 0>
